...
$$ \sum F => F(t) - c \dot x(t) - k x(t) - m \ddot x(t) = 0 $$

Where F(t) is the external force applied to the mass, c is the damping constant, k is the spring stiffness constant, m is the mass, x is the position of the mass. This systems's input is the external force and the output is the position. The external force is going against inertia, friction, and restoring force.

$$ m \ddot x = F - c \dot x - kx $$

$$ \ddot x = \frac{1}{m} (F - c \dot x - kx) $$

This is a linear time invariant (LTI) system. 2nd order differential equation can be broken down into many lower order differential equations. (Can use lsim or odeint to solve 2nd order diff eq instead of using state space)

# State Space Representation
General state space equation of a linear system.
...
State variables represent a behavior in the system i.e. coordinates, voltage/current, temperature/pressure, energy. The minimum states required are typically equal to the order of the system's defining differential equation or the denominator order of the transfer function of the system.
...
Using the model equation $\ddot x = \frac{1}{m} (F - c \dot x - kx)$, take the derivation of X
...
and only want to observe the position using $y = Cx + Du$
...
$$
\begin{aligned}
\frac{x_{1}[k+1] - x_{1}[k]} {\Delta t} = x_{2}[k]
\end{aligned}
$$
...
$$
\begin{aligned} x_2[k+1] =
\frac{-\Delta tk}{m}x_1[k] + (1 - \frac{\Delta tc}{m})x_2[k] +
\frac{\Delta t}{m}F[k]
\end{aligned}
$$
...
With output focused on position
...
# Open Loop Response
Open loop means having no feedback loop. The reference signal goes straight into the model. The model's transfer function can be found by taking the laplace transformation of the equation 

$$ F(t) - c \dot x(t) - kx(t) - m \ddot x(t) = 0 $$

$$ F(s) - csX(s) - k X(s) - ms^2X(s) = 0 $$

$$ (s) (ms^2 + cs + k) = F(s) $$

$$ \frac{X(s)}{F(s)} = \frac{1}{ms^2 + cs + k} $$

With the example for mass = 1 kg, friction = 10 N*s/m, and spring constant = 20 N/m. Input force is in N.
...
Poles at -7.2 and -2.8\
![image](plots/open_loop_pzmap.png)\
Can see that the poles also shows that this model converges to a value and will not oscillate.

When using a larger mass = 10, the model shows oscillations around 0.05.\
![image](plots/open_loop_step_response_2.png)\
![image](plots/open_loop_pzmap_2.png)\
Poles at -0.5 + 1.3i and -0.5 - 1.3i \
...
Poles at -53.1, -5.9, and -1 with zeros at -6 and -1\
...
Poles at -2.5+5.1i, -2.5-5.1i, and -0.9 with zeros at -6 and -1\
Can see the poles away from the real axis, which results in oscillations on the output.
